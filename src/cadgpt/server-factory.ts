  ...
  const server = new McpServer(
    { name: "cadgpt", version: "0.1.0" },
    {
      ...
      instructions: [
        ...
        "A user-selected external library folder is a read-only import source. library_import copies an explicitly approved source into AppData; all later CadGPT reads/writes use the managed AppData copy and never write back to the source folder.",
        "Permanent appdata/libraries/** are read-only to generic file tools. Generic writes are restricted to workspace/data; permanent Lisp/Job mutations must use controlled import/promotion tools so User Registry cannot drift from implementation.",
        ...
        "jobcreate authors/refines Jobs in appdata/workspace/job-draft/**. Existing Jobs use job_checkout; all drafts use job_draft_validate; reusable Jobs become permanent only through job_promote_draft after real test/final validation evidence and explicit user acceptance.",
        ...
        "Imported Lisp is indexed without source mutation. Only when the user activates write-lisp for an edit should lisp_checkout create a workspace draft and normalize its header/description. Blocking source syntax errors are reported but do not prevent checkout for repair.",
        ...
        "New Lisp work uses appdata/workspace/lisp-draft/**, then static validation, an explicitly approved CAD test drawing, verified load/runtime verification, and lisp_promote_draft into an existing managed AppData Lisp Library. Helper-only Lisp without public c: commands is allowed.",
