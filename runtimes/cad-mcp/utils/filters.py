...
SUPPORTED_FILTER_TYPES = {
    "line", "polyline", "circle", "arc", "ellipse", "spline", "point",
    "text", "mtext", "hatch", "wipeout", "solid", "dimension", "block",
}
DEFAULT_LIMIT = 200
MAX_LIMIT = 5000
...
def normalize_filter(filter_data: dict | None) -> dict:
    ...
    if types is not None:
        ...
        invalid = sorted(set(normalized) - SUPPORTED_FILTER_TYPES)
        if invalid:
            raise FilterError(f"Unsupported entity types: {', '.join(invalid)}")
        result["types"] = normalized
    for key in ("layer", "handle", "text_contains"):
