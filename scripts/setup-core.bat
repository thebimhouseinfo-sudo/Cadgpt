...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$p='.env'; $text=Get-Content $p -Raw; $changed=$false; if($text -match '(?m)^PORT=3000\s*if errorlevel 1 (
...
exit /b 1
){ $text=[regex]::Replace($text,'(?m)^PORT=3000\s*if errorlevel 1 (
  >>".env" echo CADGPT_BUILD_PROFILE=development
  echo [OK] Enabled development profile for this source checkout.
)
...
echo [ERROR] CadGPT setup core failed.
exit /b 1
,'PORT=3100'); $changed=$true }; if($text -match '(?m)^OPENAI_TUNNEL_HEALTH_PORT=8080\s*if errorlevel 1 (
  >>".env" echo CADGPT_BUILD_PROFILE=development
  echo [OK] Enabled development profile for this source checkout.
)
...
echo [ERROR] CadGPT setup core failed.
exit /b 1
){ $text=[regex]::Replace($text,'(?m)^OPENAI_TUNNEL_HEALTH_PORT=8080\s*if errorlevel 1 (
  >>".env" echo CADGPT_BUILD_PROFILE=development
  echo [OK] Enabled development profile for this source checkout.
)
...
echo [ERROR] CadGPT setup core failed.
exit /b 1
,'OPENAI_TUNNEL_HEALTH_PORT=8180'); $changed=$true }; if($changed){ Set-Content $p $text -Encoding UTF8; Write-Host '[OK] Migrated legacy CadGPT ports 3000/8080 -> 3100/8180.' }"
if errorlevel 1 goto :failed
