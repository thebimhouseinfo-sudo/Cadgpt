...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$repo=[System.IO.Path]::GetFullPath('%ROOT%\'); $line=Get-Content '.env' -ErrorAction SilentlyContinue | Where-Object { $_ -match '^\s*CADGPT_APPDATA_ROOT\s*=' -and -not $_.TrimStart().StartsWith('#') } | Select-Object -First 1; $configured=if($line){(($line -split '=',2)[1].Trim()).Trim([char]39).Trim([char]34)}else{''}; if($configured -eq 'appdata'){$configured=''}; $root=if([string]::IsNullOrWhiteSpace($configured)){[System.IO.Path]::GetFullPath((Join-Path $env:LOCALAPPDATA 'CadGPT'))}elseif([System.IO.Path]::IsPathRooted($configured)){[System.IO.Path]::GetFullPath($configured)}else{[System.IO.Path]::GetFullPath((Join-Path $repo $configured))}; $recovery=Join-Path $root 'state\cad-mcp-dev-recovery'; $pending=@(); if(Test-Path $recovery){$pending=@(Get-ChildItem $recovery -Directory -ErrorAction SilentlyContinue | Where-Object { -not ($_.Name.StartsWith('.') -and $_.Name.EndsWith('.tmp')) })}; if($pending.Count -gt 0){ Write-Host '[ERROR] Pending CAD MCP development recovery baseline found.' -ForegroundColor Red; Write-Host 'Run CadGPT development recovery before setup so setup cannot overwrite an unaccepted runtime.' -ForegroundColor Yellow; exit 42 }"
if errorlevel 1 (
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$repo=[System.IO.Path]::GetFullPath('%ROOT%\'); $line=Get-Content '.env' -ErrorAction SilentlyContinue | Where-Object { $_ -match '^\s*CADGPT_APPDATA_ROOT\s*=' -and -not $_.TrimStart().StartsWith('#') } | Select-Object -First 1; $configured=if($line){(($line -split '=',2)[1].Trim()).Trim([char]39).Trim([char]34)}else{''}; if($configured -eq 'appdata'){$configured=''}; $root=if([string]::IsNullOrWhiteSpace($configured)){[System.IO.Path]::GetFullPath((Join-Path $env:LOCALAPPDATA 'CadGPT'))}elseif([System.IO.Path]::IsPathRooted($configured)){[System.IO.Path]::GetFullPath($configured)}else{[System.IO.Path]::GetFullPath((Join-Path $repo $configured))}; $dirs=@('libraries\lisp','libraries\jobs','registry\user','workspace\lisp-draft','workspace\job-draft','data\runs','runtime\dynamic-lisp','drawings','state','logs'); New-Item -ItemType Directory -Force -Path $root | Out-Null; foreach($rel in $dirs){$target=Join-Path $root $rel; New-Item -ItemType Directory -Force -Path $target | Out-Null; if(-not (Test-Path $target)){Write-Error ('Could not create AppData directory: '+$target); exit 1}}; Write-Host ('[OK] CadGPT AppData root: '+$root)"
...
if errorlevel 1 (
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -Command "$line=Get-Content '.env' -ErrorAction SilentlyContinue | Where-Object { $_ -match '^\s*CADGPT_APPDATA_ROOT\s*=' -and -not $_.TrimStart().StartsWith('#') } | Select-Object -First 1; $configured=if($line){(($line -split '=',2)[1].Trim()).Trim([char]39).Trim([char]34)}else{''}; if($configured -eq 'appdata'){$configured=''}; $root=if([string]::IsNullOrWhiteSpace($configured)){Join-Path $env:LOCALAPPDATA 'CadGPT'}elseif([System.IO.Path]::IsPathRooted($configured)){$configured}else{Join-Path '%ROOT%' $configured}; $log=Join-Path $root 'logs'; foreach($n in @('tray.log','cadgpt.err.log','cadgpt.out.log','tunnel.err.log','tunnel.out.log')){ $p=Join-Path $log $n; if(Test-Path $p){ Write-Host ('--- '+$n+' ---'); Get-Content $p -Tail 30 } }"
  goto :failed
)
