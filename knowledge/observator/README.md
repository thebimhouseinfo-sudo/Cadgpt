...
3. own the minimal Drawing Anchor that binds AppData metadata to a DWG copy.
...
The Drawing Anchor is created lazily: a normal drawing has no reason to contain one until Observator first needs metadata for that drawing. The anchor is the **only CAD entity Observator may create or modify**.
