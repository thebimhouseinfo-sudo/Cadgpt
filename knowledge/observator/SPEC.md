...
- `last_revision` — the Observation revision carried by this DWG copy and used as the parent/checkpoint for its next Observation Job.
...
If the user continues working from revision 12, the next successfully completed Observation Job creates a new unique revision whose parent is 12:
...
Revision 16 is newer as a record but continues from revision 12. Revisions 13–15 remain another valid branch of the same `drawing_id` lineage.

Branch/parent relationships belong in AppData revision metadata, for example:
...
A new revision identifier must be unique within the `drawing_id`. It must not be computed as `anchor.last_revision + 1`, because a different branch may already have later revision numbers.
...
3. Allocate a new unique revision in AppData.
...
This means that the opened DWG copy carries checkpoint 12 while later Observation history also exists. It does not mean the opened drawing is invalid or that the anchor should automatically be advanced to revision 15. The user may intentionally be continuing work from the older copy.
...
→ allocate unique revision with parent_revision = parent
