...
> A completed Observation Job writes/finalizes its AppData result, then advances the Drawing Anchor `last_revision` exactly once.
...
5. Advance the Drawing Anchor `last_revision` once at successful Observation Job completion.
...
- `last_revision` — monotonically increasing Observation revision carried by this DWG copy.

Rename, move, copy, or Save As do not change `drawing_id`, `origin_name`, or `created_at`.

The anchor does not store name history, file paths, AutoCAD fingerprint/version history, semantic system metadata, entity snapshots, or Job-specific business state.
...
See `DRAWING_ANCHOR.md` for the full identity, revision, save, and crash semantics.
...
2. Job writes/updates its AppData result/log for revision N+1.
3. Job finalizes that AppData result.
4. Observator updates anchor.last_revision from N to N+1 exactly once.
5. Job is complete.
```
...
latest AppData revision             = 15
```

This means that the opened DWG copy carries checkpoint 12 while later Observation records exist in AppData. It does not mean the opened drawing is invalid or that the anchor should automatically be advanced to revision 15. The user may intentionally be opening an older version/copy.

Two copied DWGs may therefore share the same `drawing_id` while carrying different `last_revision` values. They remain versions/copies of the same logical drawing lineage.
...
→ Job finalizes AppData result for revision N+1
→ Observator advances anchor.last_revision once
→ Job completes
```
