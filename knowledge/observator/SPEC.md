...
→ resolve Drawing Anchor
   ...
   └─ not found → build drawing_id from current drawing name + creation timestamp + unique numeric suffix
...
- `drawing_id` — stable unique id generated by CadGPT and used as the AppData key. It embeds the drawing name at first anchor creation, UTC creation timestamp, and opaque numeric uniqueness suffix.
...
Creation time and origin drawing name are therefore not stored as separate anchor fields. They are immutable provenance embedded once in `drawing_id`.

Rename, move, copy, or Save As never regenerate `drawing_id`.

The unique numeric suffix is CadGPT-generated, opaque, and collision-checked within the managed AppData drawing namespace. Timestamp plus suffix provides practical uniqueness while keeping the id human-recognizable.
