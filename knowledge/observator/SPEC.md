...
→ resolve Drawing Anchor
   ...
   └─ not found → build drawing_id from current drawing name + unique numeric string
                  set created_at
...
The anchor stores only three logical fields:
...
- `drawing_id` — stable unique id generated by CadGPT and used as the AppData key. Format: `<origin-drawing-name>-<unique-numeric-string>`, for example `FCU-L03-583104927641`.
...
The drawing-name part of `drawing_id` is captured only when the anchor is first created. Rename, move, copy, or Save As never regenerate `drawing_id`.

The unique numeric suffix is opaque, CadGPT-generated, and collision-checked within the managed AppData drawing namespace. It has no business meaning.

The anchor does not store a separate origin/current drawing name, name history, file paths, AutoCAD fingerprint/version history, branch history, parent history, semantic system metadata, entity snapshots, or Job-specific business state.
